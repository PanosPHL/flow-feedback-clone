  ...
  const flows =
    useSelector((state) => {
      ...
      if (!category) {
        ...
        return flows
          .filter((flow, i) => flow.categoryId === 1)
          .filter((flow2, i) => i <= 3);
      } else if (category === 'ssbm') {
        return flows
          .filter((flow, i) => flow.categoryId === 2)
          .filter((flow2, i) => i <= 3);
      } else if (category === 'ow') {
        return flows
          .filter((flow, i) => flow.categoryId === 3)
          .filter((flow2, i) => i <= 3);
      } else {
        return flows
          .filter((flow, i) => flow.categoryId === 4)
          .filter((flow2, i) => i <= 3);
      }
    }) || [];
    ...
    >
      <div className={styles.headerContainer}>
        <img
          src={
            category === 'lol'
              ? '/images/lol-home-logo.png'
              : category === 'ssbm'
              ? '/images/ssbm-home-logo.png'
              : category === 'ow'
              ? '/images/ow-home-logo.png'
              : category === 'csgo'
              ? '/images/csgo-home-logo.png'
      ...
      <div className={styles.cardContainer}>
        {flows.length ? (
          flows.map((flow, i) => {
            return <BrowseFlowCard key={`card-${i + 1}`} i={i} flow={flow} />;
          })
        ) : (
          <></>
        )}
